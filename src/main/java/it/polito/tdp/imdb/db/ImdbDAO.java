...
public class ImdbDAO {
	...
	public List<Actor> getVertex(String genre, Map<Integer, Actor> actorsIdMap) {
		String sql = "SELECT DISTINCT r.actor_id AS id " + "FROM roles r, movies_genres g "
				+ "WHERE g.genre = ? AND r.movie_id = g.movie_id " + "ORDER BY id ";
	...
	public List<Adiacenza> getAdiacenze(String genre, Map<Integer, Actor> actorsIdMap) {
		String sql = "SELECT r1.actor_id as a1, r2.actor_id as a2, COUNT(DISTINCT r1.movie_id) as weight "
				+ "FROM roles r1, roles r2, movies_genres mg " + "WHERE r1.actor_id > r2.actor_id AND "
				+ "r1.movie_id = r2.movie_id AND " + "mg.movie_id = r1.movie_id AND " + "mg.genre = ?"
		...
		try {
			...
			while (res.next()) {
				...
				if (actorsIdMap.containsKey(res.getInt("a1")) && actorsIdMap.containsKey(res.getInt("a2"))) {
					result.add(new Adiacenza(actorsIdMap.get(res.getInt("a1")), actorsIdMap.get(res.getInt("a2")),
							res.getInt("weight")));
				}
			}
			conn.close();
